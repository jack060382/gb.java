...
public class ClientHandler {
    ...
    private final String LogDir = "logs/clients";
    ...
    public ClientHandler(Server server, Socket socket) {
        ...
        try {
            ...
            */
            ...
            logger.fatal("Something went wrong during client establishing");
    ...
    private void loadLastLocalMessages() {
        ...
        try {
            ...
            logger.fatal("Error loading last user messages");
            e.printStackTrace();
        }
    }
    ...
    private void writeLog(String msg) {
        try {
            ...
            logger.error("Writing messages log error");
            // skip
        }
    ...
    private void closeConnection() {
        ...
        try {
            ...
            logger.error("Error closing input client stream");
            e.printStackTrace();
        }
        ...
        try {
            ...
            logger.error("Error closing output client stream");
            e.printStackTrace();
        }
        ...
        new Thread(() -> {
                while (true) {
                    if (Math.floor((System.currentTimeMillis() - incomeTime)/1000) >= AUTH_TIMEOUT) {
                        ...
                        closeConnection();
                        logger.info("Client connection closed by timeout for user ["+this.getNick()+"]");
                        break;
                    }
                }
        }).start();
    ...
    private void changeNick(String credential) {
        ...
        logger.info("User ["+this.getNick()+"] trying to change nickname.");
        if (credentials.length == 2) {
            ...
            if (!maybeExistUser.isPresent()) {
                ...
                logger.info("User ["+this.getNick()+"] changed successful nickname on "+credentials[1]);
                sendMessage("Nickname changed successful on "+credentials[1]+"!");
            }
            else {
                logger.info("Fail changing nickname by user  ["+this.getNick()+"] nickname already exist.");
                sendMessage("This nickname is already use!");
            }
        }
        else {
            logger.info("Fail changing nickname by user  ["+this.getNick()+"] incorrect command.");
            sendMessage("Incorrect nickname change command!");
        }
    }

    private void doAuthentication() throws IOException {
        logger.info("New user coming");
            ...
            if (maybeCredentials.startsWith("-auth")) {
                ...
                if (maybeUser.isPresent()) {
                    ...
                    if (server.isNotUserOccupied(user.getNick())) {
                        ...
                        sendMessage("AUTH OK.");
                        sendMessage("Welcome.");
                        logger.info("User authorized login ["+user.getLogin()+"]");
                        ...
                        try {
                            ...
                            logger.error("Error opening message log for user ["+user.getLogin()+"]");
                ...
                logger.info("Invalid auth operation");
